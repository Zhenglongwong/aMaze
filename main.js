...
const unmask = () => {
	...
	$(
		`[x=${state.player_loc_x}][y=${
			state.player_loc_y + 1
		}]`
	).removeClass(MASK);
	$(
		`[x=${state.player_loc_x}][y=${
...
const createStart = (event) => {
	if (state.selectStart) {
		...
		state.player_loc_x = parseInt(
			$(PLAYERCLASS).attr("x")
		);
		state.player_loc_y = parseInt(
			$(PLAYERCLASS).attr("y")
		);
	}
};
...
const createEnd = (event) => {
	if (
		state.selectEnd &&
		!$(event.target).attr("class").includes(WALL)
	) {
...
const toggleMovement = () => {
	if (
		...
		alert(
			"Please select a start point and end point."
		);
		return;
	}
	state.draw = false;
...
const reset = () => {
	$(BOXCLASS).removeClass(
		"selected player end mask"
	);
	state = { ...BASESTATE };
	clearInterval(intervalID);
	clearInterval(timerInterval);
...
const runEventListeners = () => {
	$(BOXCLASS)
		.on("mouseover", drawMaze)
		.on("click", createEnd)
		.on("click", createStart)
...
const saveMap = (name) => {
	if (name) {
		...
		if (
			...
			alert(
				"Please select a start point and at least 1 objective."
			);
			return;
		}
		var mapName = prompt(
	...
	$(`#${mapName}`).on("click", () => {
		loadMap(localStorage.getItem(mapName));
	});
};

const deleteMaze = () => {
	let name = prompt(
		"Enter the name of the maze you want to delete:"
	);
